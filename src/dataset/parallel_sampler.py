...
class ParallelSampler():
    def __init__(self, data, args, num_episodes=None):
        ...
        if self.args.cross_domain:
            self.all_domain = np.unique(self.data['domain'])
            self.num_in_classes = 15
    ...
    def worker(self, done_queue):
        ...
        while True:
            ...
            if self.args.cross_domain:
                sampled_domain = np.random.permutation(len(self.all_domain))[0]
                tmp_class_list = list(range(self.num_classes))
                sampled_classes = np.random.permutation(tmp_class_list[sampled_domain*self.args.way: (sampled_domain+1)*self.args.way])[:self.args.way]
